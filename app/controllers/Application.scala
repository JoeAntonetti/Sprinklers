...
import models.SprinklerHTTPClient

object Articles extends Controller with MongoController {
  
  var clients = List[SprinklerHTTPClient]()
  var logins = List[Article]()
  ...
  def index = Action { implicit request =>
    ...
    found.collect[List]().map { articles =>
      articles.foreach{ article =>
        useLogin(new SprinklerHTTPClient(article.title, article.publisher))
  ...
  def useLogin(login: SprinklerHTTPClient) = {
    println(login)
  }
